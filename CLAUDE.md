...
| Movement animation (path interpolator) | [src/engine/moving_unit.py](src/engine/moving_unit.py) |
...
| Faction select screen | [src/engine/faction_select_view.py](src/engine/faction_select_view.py) |
| Procedural chiptune audio (music + SFX) | [src/engine/audio.py](src/engine/audio.py) — WAVs generated to `assets/audio/` on first run |
...
- **View vs. current player.** `GameView.view_player_id` is the **human** player (what the user sees — whose fog we render). `state.current_player_id` is whose turn it is. The two differ during the AI's turn: the HUD reads `current_player_id` (so you know it's the AI's turn), but rendering reads `view_player_id` (so you can't see enemy units moving outside your own fog). After AI actions, recompute fog for `view_player_id`, not the current player.
- **Idle animation.** `GameView._anim_time` accumulates seconds via `on_update(delta_time)`. Pass it to `sprites.draw_unit(..., anim_time=...)`. Units get a subtle sine-wave bob; wyverns flap wings (scale), heroes get flicker/swirl effects. Spent units (`has_acted=True`) stay animated but dimmed (color carries the "spent" signal).
- **Event queue + animations.** Any batch of events coming out of `apply_action` is enqueued on `GameView._event_queue` and drained one at a time. `Move` events with a real path kick off a `MovingUnit` (path-based interpolator at 0.09 s/tile). `Attack` events trigger the `FightScene`. AI is paused while either animation is playing. Click-attack emits `[Move, Attack]` — the move animates first, then the fight. When adding a new event type that should gate gameplay on an animation, queue it the same way.
- **Non-convex polygons.** `arcade.draw_polygon_filled` uses a triangle fan, which misdraws concave or self-intersecting polygons (you'll see stray triangles leaking through the fill). Keep filled polygons convex. Split concave shapes into two convex triangles, or render zigzag shapes (lightning, etc.) as thick line segments.
- **Capture = attack the building.** There is no CaptureAction; buildings have HP and flip ownership when attacked to 0 HP. Pathfinding treats enemy/neutral buildings as impassable (must be attacked), own buildings as passable. Units that just flipped a building cannot produce from it that turn (`has_produced` set so the new owner waits a turn).
